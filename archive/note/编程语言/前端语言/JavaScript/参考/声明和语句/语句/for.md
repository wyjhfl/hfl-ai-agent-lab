...
注意，`for` 语句后面必须使用分号，因为它是一个[空语句](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Statements/Empty)。否则，`for` 语句将 `console.log` 行作为 `statement` 部分，这会导致 `log` 被执行多次。
