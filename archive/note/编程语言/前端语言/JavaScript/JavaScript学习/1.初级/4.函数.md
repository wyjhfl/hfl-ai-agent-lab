...
显然，_函数本身就是对象_，因此这些对象也有方法（参见 [`Function`](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Function) 对象）。
...
使用 _默认参数_ ，在函数体的手动检查就不再必要了。现在，你可以在函数头简单地把 `1` 设定为 `b` 的默认值：
```js
