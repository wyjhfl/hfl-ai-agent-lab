...
确定响应何时是 fresh 的和何时是 stale 的标准是 **age**。在 HTTP 中，age 是自响应生成以来经过的时间。这类似于其他缓存机制中的 [TTL](https://developer.mozilla.org/zh-CN/docs/Glossary/TTL)。
...
`Expires` 标头使用**明确的时间**而不是通过指定**经过的时间**来指定缓存的生命周期。
```
...
## [参见](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Caching#%E5%8F%82%E8%A7%81)
