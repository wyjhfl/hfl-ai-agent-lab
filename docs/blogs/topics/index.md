...
| 想做 RAG 项目 | [RAG 项目面试表达](/topics/rag-project-interview) → [RAG 工程化](/topics/rag-engineering-system) → [RAG Query Router](/note/Engineering/rag-query-router) → [RAG Ingestion Quality Gate](/note/Engineering/rag-ingestion-quality-gate) → Evaluation / Trace 相关内容 |
...
| 想做生产级 Agent | [Agent 开发 Playbook](/topics/agent-development-playbook) → [Agent Runtime](/topics/agent-runtime-explained) → [Agent Autonomy Levels](/note/Engineering/agent-autonomy-levels) → [Agent Workflow 状态机](/note/Engineering/agent-workflow-state-machine) → [Tool Risk Classification](/note/Engineering/tool-risk-classification) → [Tool Idempotency Side Effect](/note/Engineering/tool-idempotency-side-effect) → [Agent 配置治理](/note/Engineering/agent-configuration-management) → [Agent Approval Workflow](/note/Engineering/agent-approval-workflow) → [Model Provider Failover](/note/Engineering/model-provider-failover) → [Agent Latency Budget](/note/Engineering/agent-latency-budget) → [LLM Request Ledger](/note/Engineering/llm-request-ledger) → [Agent SLO 与 Error Budget](/note/Engineering/agent-slo-error-budget) → [生产级 Agent Readiness Review](/topics/agent-production-readiness-review) → [Agent Release Gate](/note/Engineering/agent-release-gate) |
...
| 想优化线上 Agent | [Agent Run Replay](/note/Engineering/agent-run-replay) → [Agent 错误分类](/note/Engineering/agent-error-taxonomy) → [Agent Feedback Triage](/note/Engineering/agent-feedback-triage) → [Agent Queue 与 Backpressure](/topics/agent-queue-backpressure) → [LLM Semantic Cache](/note/Engineering/llm-semantic-cache) → [Prompt Regression Testing](/note/Engineering/prompt-regression-testing) → [Eval Drift Monitoring](/note/Engineering/eval-drift-monitoring) → [Agent 故障演练](/note/Engineering/agent-production-failure-drill) → [Agent SLO 与 Error Budget](/note/Engineering/agent-slo-error-budget) → [Model Provider Failover](/note/Engineering/model-provider-failover) → [Agent 生产运维 Runbook](/note/Engineering/agent-production-ops-runbook) |
...
| 想整理求职作品集 | [AI Agent 求职作品集路线](/topics/ai-agent-portfolio-roadmap) → [AI Agent Career Readiness Checklist](/topics/ai-agent-career-readiness-checklist) → [AI Agent 作品集 Case Study 模板](/topics/ai-agent-portfolio-case-study-template) → [AI Agent Demo Acceptance Script](/topics/ai-agent-demo-acceptance-script) → [AI Agent Interview Story Bank](/topics/ai-agent-interview-story-bank) → [AI Agent 项目答辩稿](/topics/ai-agent-project-defense-script) → [AI Agent Offer Portfolio Review](/topics/ai-agent-offer-portfolio-review) → [项目实战](/projects) → [简历描述模板](/note/Interview/resume-bullets) |
