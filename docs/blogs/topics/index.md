...
| 想做 RAG 项目 | [RAG 项目面试表达](/topics/rag-project-interview) → [RAG 工程化](/topics/rag-engineering-system) → [RAG Ingestion Quality Gate](/note/Engineering/rag-ingestion-quality-gate) → Evaluation / Trace 相关内容 |
...
| 想做生产级 Agent | [Agent 开发 Playbook](/topics/agent-development-playbook) → [Agent Runtime](/topics/agent-runtime-explained) → [Agent Autonomy Levels](/note/Engineering/agent-autonomy-levels) → [Agent Workflow 状态机](/note/Engineering/agent-workflow-state-machine) → [Tool Risk Classification](/note/Engineering/tool-risk-classification) → [Agent 配置治理](/note/Engineering/agent-configuration-management) → [Agent Approval Workflow](/note/Engineering/agent-approval-workflow) → [Model Provider Failover](/note/Engineering/model-provider-failover) → [Agent Latency Budget](/note/Engineering/agent-latency-budget) → [Agent SLO 与 Error Budget](/note/Engineering/agent-slo-error-budget) → [生产级 Agent Readiness Review](/topics/agent-production-readiness-review) → [Agent Release Gate](/note/Engineering/agent-release-gate) |
...
| 想优化线上 Agent | [Agent Run Replay](/note/Engineering/agent-run-replay) → [Agent 错误分类](/note/Engineering/agent-error-taxonomy) → [Agent Queue 与 Backpressure](/topics/agent-queue-backpressure) → [LLM Semantic Cache](/note/Engineering/llm-semantic-cache) → [Prompt Regression Testing](/note/Engineering/prompt-regression-testing) → [Eval Drift Monitoring](/note/Engineering/eval-drift-monitoring) → [Agent 故障演练](/note/Engineering/agent-production-failure-drill) → [Agent SLO 与 Error Budget](/note/Engineering/agent-slo-error-budget) → [Model Provider Failover](/note/Engineering/model-provider-failover) → [Agent 生产运维 Runbook](/note/Engineering/agent-production-ops-runbook) |
| 想学习 AI 编程协作 | [Claude Code 实战工作流](/topics/claude-code-workflow) → [Skills 编写](/note/AI-Tools/skill-authoring) → [Skill 运营手册](/note/AI-Tools/skill-operations-playbook) → [MCP Observability Metrics](/note/Engineering/mcp-observability-metrics) → [MCP Token Exchange](/note/Engineering/mcp-token-exchange) → [MCP Tool Schema 设计](/note/Engineering/mcp-tool-schema-design) → [MCP Version Deprecation](/note/Engineering/mcp-version-deprecation) → [MCP Server Hardening](/note/Engineering/mcp-server-hardening) → [AI 编程审查清单](/topics/ai-coding-review-checklist) |
...
| 想做商业化产品 | [Agent SaaS 产品化](/topics/agent-saas-productization) → [Agent SaaS 多租户、RBAC 与配额设计](/topics/agent-saas-tenant-rbac-quota) → [LLM Cost Chargeback](/note/Engineering/llm-cost-chargeback) → [LLM Cost Anomaly Detection](/note/Engineering/llm-cost-anomaly-detection) → [Agent Product Metrics](/topics/agent-product-metrics) → [Agent UI 产品化设计](/topics/agent-ui-product-design) |
