---
layout: home

hero:
  name: HFL AI Agent Lab
  text: AI Agent 工程知识库
  tagline: 学什么 · 怎么做 · 怎么答 — 系统沉淀 AI Agent 工程能力的个人知识库与作品集
  actions:
    - theme: brand
      text: 进入学习路线
      link: /note/AI-Agent/
    - theme: alt
      text: 面试题库
      link: /note/AI-Interview/
    - theme: alt
      text: 工程化笔记
      link: /note/Engineering/

features:
  - icon: 🤖
    title: AI Agent 学习路线
    details: 从 Agent 基础到 Tool Calling、LangGraph、Multi-Agent、Evaluation，11 个阶段的工程学习地图。专题文章《从 RAG 到 Agent Harness》已发布。
    link: /note/AI-Agent/
    linkText: 查看路线 →

  - icon: 🧩
    title: Agent 源码拆解
    details: 拆解 Hermes Agent、Harness Engineering、OpenClaw，提炼可迁移的架构设计和工程模式。
    link: /note/Source-Reading/
    linkText: 阅读拆解 →

  - icon: 🛠️
    title: 工程化笔记
    details: FastAPI、RAG 工程化、Docker、可观测性、Agent Trace、MCP Server、上线检查清单。
    link: /note/Engineering/
    linkText: 工程笔记 →

  - icon: 🎯
    title: 面试题库
    details: Agent、RAG、LLM 工具调用、大模型工程、LangChain / LangGraph 五大方向，第一轮原创题解已完成。
    link: /note/AI-Interview/
    linkText: 查看题库 →

  - icon: 🚀
    title: 项目实战
    details: 项目 A（RAG 工单系统）与项目 B（多 Agent Copilot）入口已预留，后续逐步展开。
    link: /projects
    linkText: 查看项目 →

  - icon: 📌
    title: 当前建设阶段
    details: v2.3 阶段：Hermes Agent 高级用法与进阶玩法专题完成。新增子代理驱动开发、Kanban 多代理协作、TDD、预提交审查、MCP、Cron、Skills、安全控制、TUI、Worktree 与跨平台网关等高级能力整理，项目 B 保持占位。
    link: /blogs/topics/rag-to-agent-harness
    linkText: 阅读专题 →
---
